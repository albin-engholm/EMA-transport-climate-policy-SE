...
if __name__ == "__main__":
    ...
    model = ExcelModel("scenarioModel", wd="./models",
                       model_file='trv_scenario_AV_v3_IK.xlsx')
   ...
   #Set constants
    model.constants = [Constant("C22", -.9)] #Adjust truck price elasticity
    model.constants = [Constant("C12", -.1)] #Adjust truck price elasticity
    ...
    from ema_workbench.em_framework import samplers
    #Simulation settings
    nr_scenarios=n_sobol  #select number of scenarios (per policy)
    run_with_policies=1
    use_multi=1
    n_p=3
