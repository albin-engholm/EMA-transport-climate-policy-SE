...
date='2023-12-14'#Specify date the MORDM MOEA was started
date_archive="2023-12-14"
nfe_MOEA=50000 #Specify the number of nfes used for the MORDM MOEA
                

all_archives = []

for policy_type in policy_types:
    archives = ArchiveLogger.load_archives(f"./archives/{str(nfe_MOEA)}_{policy_type}_{date_archive}.tar.gz") #load archive
    ...
    if len(metrics) > 0:
        pickle.dump(metrics_by_seed,open(f"./output_data/{str(nfe_MOEA)}_{policy_type}_{date_archive}_convergence_metrics.p","wb"))
#%% visualize metrics
for policy_type in policy_types:

    metrics_by_seed=pickle.load(open(f"./output_data/{str(nfe_MOEA)}_{policy_type}_{date_archive}_convergence_metrics.p","rb"))
