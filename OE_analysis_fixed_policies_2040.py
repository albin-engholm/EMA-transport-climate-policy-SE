...
if load_results==1:
    ...
    #%% Check if electrification rate is used as external uncertianty
    if "Car el share" in df_full.columns:
...
if External_electrification_rate:
...
sns.scatterplot(y='CO2 TTW change light vehicles', 
                x='Driving cost light vehicles relative reference', 
              data=df_full, hue="policy", alpha=0.5)
plt.show()

sns.scatterplot(y='CO2 TTW change trucks', 
                x='Driving cost trucks relative reference', 
              data=df_full, hue="policy", alpha=0.5)
plt.show()

sns.scatterplot(y='CO2 TTW change trucks', x='VKT trucks', 
              data=df_full, hue="policy", alpha=0.5)
plt.show()
sns.scatterplot(y='CO2 TTW change light vehicles', x='VKT light vehicles', 
              data=df_full, hue="policy", alpha=0.5)
plt.show()
...
sns.scatterplot(y='Delta CS tax', x='CO2 TTW change total', 
              data=df_full, hue="policy",  alpha=0.5)
plt.show()

sns.scatterplot(y='Driving cost light vehicles relative reference', x='Energy bio total', 
              data=df_full, hue="policy",  alpha=0.5)
plt.show()
...
sns.displot(x='Driving cost light vehicles relative reference', 
            data=df_full.drop(df_full[df_full.policy=="Reference - planned policies"].index), 
            hue="policy",kde=True)

sns.displot(x='Driving cost trucks relative reference',
            data=df_full.drop(df_full[df_full.policy=="Reference - planned policies"].index),
            hue="policy",
            kde=True)
...
sns.displot(x='Delta CS total', 
            data=df_full.drop(df_full[df_full.policy=="Reference - planned policies"].index),
            hue="policy",kde=True)
sns.displot(x='Delta tax income total', 
            data=df_full.drop(df_full[df_full.policy=="Reference - planned policies"].index), 
            hue="policy",kde=True)
sns.displot(x='Delta CS tax', 
            data=df_full.drop(df_full[df_full.policy=="Reference - planned policies"].index),
            hue="policy",kde=True)
...
grid = sns.FacetGrid(data=df_full.drop(df_full[df_full.policy=="Reference - planned policies"].index),
                     col="policy",hue="policy",
                     col_wrap=3, height=6, sharex=True,sharey=True)
...
plt.figure()
sns.boxplot(x='policy', y='Driving cost light vehicles relative reference', 
            data=df_full.drop(df_full[df_full.policy=="Reference - planned policies"].index),)
...
plt.figure()
sns.boxplot(x='policy', y='Driving cost trucks relative reference', 
            data=df_full.drop(df_full[df_full.policy=="Reference - planned policies"].index),)
# plt.axhspan('Driving cost light vehicles relative reference', max(outcomes['Driving cost light vehicles relative reference']), 
#             facecolor='red', alpha=0.05,edgecolor='None')
# plt.axhline(fail_criterion_cost_light,color="red", 
...
fs_discovery, alg = feature_scoring.get_ex_feature_scores(x.drop(columns="policy"), 
                                                          y, mode=feature_scoring.RuleInductionType.CLASSIFICATION)
...
fs_discovery, alg = feature_scoring.get_ex_feature_scores(x.drop(columns=model.levers._data.keys()), 
                                                          y, mode=feature_scoring.RuleInductionType.CLASSIFICATION)
fs_discovery.sort_values(ascending=False, by=1)
plt.figure()
fig=sns.heatmap(fs_discovery, cmap='viridis', annot=True)
...
#%%%#Perform PRIM analysis for B policies
#Set up PRIM

x_b_elements=x.policy.str.startswith("B")
x_b=x[x_b_elements]
x_b_elements_index=x_b_elements[x_b_elements==True].index
y_b=y[x_b_elements_index]

prim_alg = prim.Prim(x_b, y_b, threshold=0.5)

#Find 1st box
box1 = prim_alg.find_box()
plt.title("PRIM - B policies")
#Visualizations of Box1
box1.show_tradeoff()
...
#Choose point for inspection of box 1
i1=round((len(box1.peeling_trajectory.T.columns)-1))
#or choose box manually
i1=20
box1.inspect(i1)
...
#%%%#Perform PRIM analysis for C policies
#Set up PRIM

x_b_elements=x.policy.str.startswith("C")
x_b=x[x_b_elements]
x_b_elements_index=x_b_elements[x_b_elements==True].index
y_b=y[x_b_elements_index]
...
#%%%#Perform PRIM analysis for D policies
#Set up PRIM

x_b_elements=x.policy.str.startswith("D")
x_b=x[x_b_elements]
x_b_elements_index=x_b_elements[x_b_elements==True].index
y_b=y[x_b_elements_index]
