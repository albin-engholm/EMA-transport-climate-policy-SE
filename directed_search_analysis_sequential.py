...
cols_to_cluster = ['CO2 TTW change total', 'Energy bio total',
                   'Energy el total', 'Driving cost light vehicles', 
                   'Driving cost trucks']
...
for name, cluster in df_scaled.groupby(["Scenario", "Policy type"]):
    ...
    name_str = scenario_str + policy_str
...
cols_to_plot = ['cluster', 'CO2 TTW change total', 'Energy bio total'
                ,'Energy el total', 'Driving cost light vehicles', 'Driving cost trucks']
...
for outcome in outcome_keys:
    if (model.outcomes[outcome].kind!=0) and (outcome!="CO2 TTW change total"):
        ...
        g=sns.scatterplot(data=df_full,x=outcome,y="CO2 TTW change total",hue="Scenario",style="Policy type",palette=scenario_colors)
...
for outcome in outcome_keys:
    if (model.outcomes[outcome].kind!=0) and (outcome!="CO2 TTW change total"):
        ...
        plt.rcParams['figure.dpi'] = 300
        sns.set_theme(style="whitegrid")
        g=sns.scatterplot(data=df_full,x=outcome,y="CO2 TTW change total",hue="Policy type",style="Scenario",palette=policy_colors)
        plt.axhline(y=-0.9,color="black",linestyle="--")
        g.set(ylim=[0,-1])
