...
#%% Visualize k-medoids and kmeans clusters
    ...
    for ax, (cluster, centroid) in zip(g.axes, centroids.groupby("Kmedoids cluster")):
        ...
        ax.scatter(centroid["M2_driving_cost_car"], centroid["M3_driving_cost_truck"], s=100, edgecolor='r', facecolors='none', linewidth=2, marker='o')
...
#%%  SCORIN G BASED FILTERING METHODS
    ...
    for policy_type in df_full_no_TRV["Policy type"].unique():       
        df_candidate_policies_sampled = pd.concat([df_candidate_policies_sampled,filtered_df[filtered_df["Policy type"]==policy_type].sample(n_clusters)])
